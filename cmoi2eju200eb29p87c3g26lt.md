...
datePublished: 2026-04-28T03:26:55.469Z
cuid: cmoi2eju200eb29p87c3g26lt
...
So only two months later am I starting to hold myself accountable for doing write ups to really understand. The flow is subject to change but right now I am thinking I will start with a Stack based Buffer Overflow and then add a different memory protection mechanism to my code. To where eventually a stack canary, DEP and ASLR will be enabled when compiled and I will go through the different ways that these protections can be bypassed. Finally, I will dive into the Heap and the real fun stuff. During this journey I will write my own vulnerable programs to highlight the vulnerabilities and walk through the exploit that would be leveraged to talk advantage of the vulnerability in the code.

* * *

**What is the Stack?**

The stack is a location within memory that operates in a Last in First Out (LIFO) and is typically used for short term data storage for a program. Functions primarily use the stack for both local variable storage as well as parameters for functions. When a new function is called, a new stack frame is created on the stack. This stack frame consists of the function parameters (inputs to the function), a return address (where the program should return to once the function has completed execution), and a buffer to contain data relevant to that function (static or dynamically defined variables). The stack grows up towards lower values. If this is confusing, you are not alone, it can be. However, to demonstrate the creation of the stack frame we can walkthrough a simple example.
...
![image](https://github.com/user-attachments/assets/380eaa42-3a25-4eff-a2a8-3bba82b77345 align="center")

*   This shows us the main() function calling get\_data(), and then the get\_data() function calling both scanf() and printf().
...
![image](https://github.com/user-attachments/assets/db2bffc9-3082-4701-b5fd-1383b4139f5d align="center")

*   Next we can step through the program using either si or ni .
    
*   I will be using si for the majority of this walkthrough as it allows me to step into (si) other functions, this will allow us to witness the creation of the stack frame for the get\_data() function.
    

Prior to get\_data() being called the stack frame for main() is created and it looks like the following:

![image](https://github.com/user-attachments/assets/256a9a9e-5e38-469f-9d81-3b464ddc4cee align="center")
...
![image](https://github.com/user-attachments/assets/58ba0ee4-ca05-40fa-afef-ddad76c20893 align="center")

![image](https://github.com/user-attachments/assets/eb2e26d1-5bef-4af9-85ce-7d05a42edd8e align="center")
...
![image](https://github.com/user-attachments/assets/c35b86ee-0094-45e4-8bb7-decbd301ab58 align="center")

Then we move rsp into rbp to set base pointer of the new stack frame as the value of the current stack pointer.

![image](https://github.com/user-attachments/assets/b9094b22-16b3-4e76-b3af-cb8bead05c92 align="center")
...
![image](https://github.com/user-attachments/assets/1ad35bd3-d27a-43ee-b798-2e4c37f6b8e1 align="center")
...
![image](https://github.com/user-attachments/assets/c7874af2-464f-474d-a442-0f8adfdb636f align="center")
...
We are going to skip the stack creation and walk through the program from the get\_data() function. We can again do this using break in gdb.

![image](https://github.com/user-attachments/assets/4c0edc6e-80a0-45c0-9edb-6ebc4085738f align="center")

![image](https://github.com/user-attachments/assets/4edf4913-3d79-4fd9-af04-81a5fd838805 align="center")
...
Next the program sets up the registers for the execution and return value of the scanf() function. The following screenshots show the following chain of events. 1.) The address of the top of the stack is saved into rax, which contains the memory address of the local variable that will store the input of scanf(). This indicates that the input supplied by the user will be written to the memory address assigned to rax. 2.) Move the value of raxinto rsi to set the second argument of the scanf() function to the location where scanf() will store data. 3.) Set the first argument for the scanf() function %sin the rdi register.

*   This is where the vulnerability is. By %s not having a defined amount of bytes that it can accept, the user can enter any amount of data and it will be accepted. 4.) Set rax to zero. This will allow for only the written data to be in the register and nothing from previous functions.
    

![image](https://github.com/user-attachments/assets/f4c73f65-df03-452c-9b3b-6907ca0a64c9 align="center")

![image](https://github.com/user-attachments/assets/242987cc-71e7-4a45-be35-2f8eea9a0a44 align="center")

![image](https://github.com/user-attachments/assets/099a0f29-34e0-4a24-bbce-9bec6855ffd5 align="center")

![image](https://github.com/user-attachments/assets/161a8831-ffdc-4ebd-b88d-df96d4d16665 align="center")

Here we enter the string `whoami` to demonstrate that the value is written to the stack as a local variable.

![image](https://github.com/user-attachments/assets/1d595107-a29d-4898-a961-d609759604e4 align="center")
...
![image](https://github.com/user-attachments/assets/e7672c42-8240-4402-b043-32f995a14abd align="center")
...
![image](https://github.com/user-attachments/assets/5893d255-e30e-4435-8184-fb4a94663006 align="center")
...
![image](https://github.com/user-attachments/assets/134b88ae-a574-46a0-9287-9c0f58d42341 align="center")
...
Below I stopped at the `ret` command, which normally would return us to our `main+18` location. However, here we see that our return addres is all A's. Since this is not a legitimate address our program crashes and fails. Ok cool well that is neat, but how can this be exploited?

![image](https://github.com/user-attachments/assets/e05b2fca-3805-470d-8318-6369bcea8fb1 align="center")
...
![image](https://github.com/user-attachments/assets/75468b20-158a-4042-8ea2-9d8976332a2b align="center")
...
1.)An OFFSET to reach the return address. This can literally be anything, but for ease I use A's. 2.) The memory address of a trampoline instruction. A trampoline instruction in this case is any instruction in the code that will allow us to get back to `rsp`
...
*   For demonstration purposes I included a jmp rsp instruction in the program, but often times programs are large enough where this does not need to be artificially created. 3.) Because after the program returns to `rsp` it will execute the remaining shellcode that is written to the stack.
...
![image](https://github.com/user-attachments/assets/41bbf8ac-925b-46f9-bd12-8624ab61d621 align="center")
...
![image](https://github.com/user-attachments/assets/82d692b2-e531-4090-85b5-545551368be3 align="center")
...
![image](https://github.com/user-attachments/assets/4b6a1125-fd11-458b-9c84-0ac26d366ef1 align="center")

Here we see that the stack is overwritten with a bunch of A's, but there is a valid memory address in the return address. GEF and GDB do a goodjob here to show us that the instruction associated with the address is `jmp rsp`. Under that address is a bunch of data, that looks a lot like the shellcode that we want executed. But how can we prove that? Well we can run the commands supplied to our script manually to verify.

![image](https://github.com/user-attachments/assets/353700f1-1a66-4d14-bd28-35405dff56c8 align="center")

Now we are sending the byte values here and `jhH` translates to `6a 68 48`.

![image](https://github.com/user-attachments/assets/99b63cf7-4595-4ba6-b205-2d6c74e06e44 align="center")
...
![image](https://github.com/user-attachments/assets/7c2afa0e-f4c0-4a2a-a125-d8f45c57f39a align="center")
...
![image](https://github.com/user-attachments/assets/9d020b12-74b3-4e58-a861-d06aefda913a align="center")

![image](https://github.com/user-attachments/assets/a02f2d61-1e0e-4602-a739-9dd7aaed5973 align="center")
...
![image](https://github.com/user-attachments/assets/7fcd7927-c692-4e59-b37d-74ecda5e7571 align="center")

It looks like we crashed, but in reality if we go back to the original terminal we have code execution.

![image](https://github.com/user-attachments/assets/7b7e44c1-7d08-4292-ba74-0565b25bf440 align="center")

![image](https://github.com/user-attachments/assets/f16868d0-c392-4833-bc48-2ac946a4b23e align="center")
